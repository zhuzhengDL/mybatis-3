...
public class PooledDataSource implements DataSource {
  ...
  /**
   * 在任意时间可以存在的活动（也就是正在使用）连接数量  最大活跃连接数
   */
  protected int poolMaximumActiveConnections = 10;
  /**
   * 任意时间可能存在的空闲连接数  最大空闲连接数
   */
      ...
      synchronized (state) {
        ...
        if (!state.idleConnections.isEmpty()) {
          ...
          if (state.activeConnections.size() < poolMaximumActiveConnections) {
            ...
            // 创建新的 PooledConnection 连接对象 通过dataSource获取真实连接生成PooledConnection
  ...
  protected boolean pingConnection(PooledConnection conn) {
    ...
    if (result && poolPingEnabled && poolPingConnectionsNotUsedFor >= 0
        && conn.getTimeElapsedSinceLastUse() > poolPingConnectionsNotUsedFor) { // 判断是否长时间未使用。若是，才需要发起 ping
      try {
        if (log.isDebugEnabled()) {
